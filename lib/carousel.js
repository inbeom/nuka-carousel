...
var Carousel = _react2['default'].createClass({
  ...
  setDimensions: function setDimensions(props) {
    ...
    for (var i = currentSlide; i < currentSlide + (props.slidesToShow || 1); i++) {
      var node = frame.childNodes[0].childNodes[i];
      node && showingSlides.push(node);
    }

    if (showingSlides.length > 0) {
      console.log(showingSlides.map(function (slide) {
        return slide.offsetHeight;
      }));
      console.log(Math.max(showingSlides.map(function (slide) {
        return slide.offsetHeight;
      })));
      slideHeight = this.props.vertical ? showingSlides.reduce(function (sum, slide) {
        return sum + slide.offsetHeight;
      }, 0) : Math.max.apply(null, showingSlides.map(function (slide) {
